...
class PortfolioService:
    ...
    async def snapshot(self) -> PortfolioSnapshot:
        try:
            ...
            raise AppError("Portfolio service", "loading current portfolio snapshot", f"{type(exc).__name__}: {exc}", exc) from exc
    ...
    async def refresh_snapshot(self) -> PortfolioSnapshot:
        try:
            ...
            raise AppError("Portfolio service", "refreshing provider snapshots", f"{type(exc).__name__}: {exc}", exc) from exc
    ...
    async def dashboard(self, display_currency: str) -> DashboardData:
        try:
            ...
            raise AppError("Portfolio service", f"building dashboard in {display_currency}", f"{type(exc).__name__}: {exc}", exc) from exc
